...
public class Aoa_score extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_aoa_score);
        Displayscore();
    }

    private void Displayscore() {
        String id = FirebaseAuth.getInstance().getCurrentUser().getUid();
        pRef = FirebaseDatabase.getInstance().getReference().child("Test scores").child(id).child("aoa");
        pdfRecyclerView = findViewById(R.id.recyclerviewaoa_score);
        pdfRecyclerView.setHasFixedSize(true);
        pdfRecyclerView.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false));

        progressBar = findViewById(R.id.progressbar_aoascore);
        ...
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    ...
                    progressBar.setVisibility(View.GONE);
                    Toast.makeText(Aoa_score.this, "Not attempted any quiz yet", Toast.LENGTH_SHORT).show();
                }
            }
