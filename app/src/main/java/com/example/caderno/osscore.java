...
public class osscore extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_osscore);
        Displayscore();
    }
    private void Displayscore() {
        String id = FirebaseAuth.getInstance().getCurrentUser().getUid();
        pRef = FirebaseDatabase.getInstance().getReference().child("Test scores").child(id).child("os");
        pdfRecyclerView = findViewById(R.id.recyclerviewos_score);
        pdfRecyclerView.setHasFixedSize(true);
        pdfRecyclerView.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false));

        progressBar = findViewById(R.id.progressbar_osscore);
        ...
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    ...
                    progressBar.setVisibility(View.GONE);
                    Toast.makeText(osscore.this, "Not attempted any quiz yet", Toast.LENGTH_SHORT).show();
                }
            }
